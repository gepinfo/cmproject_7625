...
export class clientController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/client/:id', this.GpDelete);
this.router.get('/client/get/search', this.GpSearch);
this.router.put('/client/get/update', this.GpSearchForUpdate);
this.router.put('/client', this.GpUpdate);
this.router.get('/client', this.GpGetAllValues);
this.router.post('/client', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into clientController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.CLIENTURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.CLIENTURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.CLIENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.CLIENTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
