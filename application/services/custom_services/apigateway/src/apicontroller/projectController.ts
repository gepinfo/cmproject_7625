...
export class projectController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/project/:id', this.GpDelete);
this.router.get('/project/get/search', this.GpSearch);
this.router.put('/project/get/update', this.GpSearchForUpdate);
this.router.put('/project', this.GpUpdate);
this.router.get('/project', this.GpGetAllValues);
this.router.post('/project', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into projectController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.PROJECTURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.PROJECTURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.PROJECTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.PROJECTURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
