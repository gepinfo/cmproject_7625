...
export class planController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/plan/:id', this.GpDelete);
this.router.get('/plan/get/search', this.GpSearch);
this.router.put('/plan/get/update', this.GpSearchForUpdate);
this.router.put('/plan', this.GpUpdate);
this.router.get('/plan', this.GpGetAllValues);
this.router.post('/plan', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into planController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.PLANURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.PLANURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.PLANURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.PLANURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
